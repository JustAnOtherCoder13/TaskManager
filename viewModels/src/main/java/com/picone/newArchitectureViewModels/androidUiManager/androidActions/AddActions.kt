...
object AddActions {
    ...
    data class AddScreenAddNewItemOnOkButtonClicked(
        val selectedItemType: String,
        val editedTask: Task?,
        val editedProject: Project?
    ) : AddAction

    data class NavigateToDetailOnAddTaskComplete(
        override val androidNavActionManager: AndroidNavActionManager
        ) : AddNavAction

    data class NavigateToProjectOnProjectComplete(
        override val androidNavActionManager: AndroidNavActionManager
        ) : AddNavAction

    data class NavigateToHomeOnUpdateTaskComplete(
        override val androidNavActionManager: AndroidNavActionManager) : AddNavAction
